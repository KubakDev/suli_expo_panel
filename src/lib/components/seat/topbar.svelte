...
<div class="px-2 flex justify-between bg-[#333333] text-[#fff] border-b border-[#fff] h-16">
	<button
	...
	<div class="flex justify-between">
		<div class="mx-2 flex justify-center items-center customShape">
			{#each customShapes as shape}
				<div
					class="h-full w-12 flex justify-center items-center px-2 rounded hover:bg-[#111111] cursor-pointer"
					on:click={() => createCustomShape(shape)}
				>
					<div class={`${shape}  border-2 border-[#fff] cursor-pointer hover:bg-[#111111] `} />
				</div>
				<Tooltip
					arrow={true}
					type="custom"
					...
					placement="bottom">{`Add ${shape}`}</Tooltip
				>
			{/each}
		</div>
		...
		>
			<svg class="w-7 h-7" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
				><g id="SVGRepo_bgCarrier" stroke-width="0" /><g
					...
					stroke-linejoin="round"
				/><g id="SVGRepo_iconCarrier">
					<path d="M4 4L20 20" stroke="#fff" stroke-width="1" stroke-linecap="round" />
					<path
						fill-rule="evenodd"
						clip-rule="evenodd"

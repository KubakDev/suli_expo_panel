<script lang="ts">
		...
		for (let file of fileInput!.files!) {
			...
			reader.onloadend = () => {
				...
				if (file.type === 'application/pdf') {
					imgSource = ImgSourceEnum.PdfLocal;
				}
	...
	function deleteImage(index: number) {
		...
		if (
			images[index].imgSource === ImgSourceEnum.PdfLocal ||
			images[index].imgSource === ImgSourceEnum.PdfRemote
		) {
...
<div class="w-full h-80 bg-[#e4e4e4] rounded-lg p-5 flex flex-wrap">
	...
	{#each images as image, index}
		<div class="h-24 w-24 bg-[#f1f3f4] mx-2 rounded-lg relative">
			...
			<div class="object-cover h-full w-full flex flex-col items-center justify-center">
				{#if image.imgSource === ImgSourceEnum.PdfRemote}
					<svg
						version="1.1"
						id="_x35_"
					...
					>
					{image.fileName.substring(image.fileName.lastIndexOf('_') + 1)}
				{/if}
			</div>
		</div>
	{/each}

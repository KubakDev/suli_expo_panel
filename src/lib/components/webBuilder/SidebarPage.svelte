<script lang="ts">
	...
	<div class="flex justify-between items-center w-full px-5 py-4">
		<div class="flex items-center gap-2">
			<input
				type="checkbox"
				class="w-4 h-4 bg-gray-100 border-gray-300 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600"
				bind:checked={showHomeStatus}
				on:change={updateStatus}
			/>
			<span class="text-sm text-gray-600 dark:text-gray-300">Show this page on home</span>
		</div>

		<!-- Update button with Tabler icon -->
		<button
			on:click={() => update()}
			class="flex items-center gap-2 py-2 px-4 rounded-md text-white bg-primary-600 hover:bg-primary-700 dark:bg-primary-600 dark:hover:bg-primary-700 transition-all duration-200 ease-in-out shadow-sm"
		>
			<IconDeviceFloppy size={20} />
			<span class="text-sm font-medium">Update Theme</span>
		</button>
	</div>

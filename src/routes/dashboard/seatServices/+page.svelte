...
<div class="max-w-screen-2xl mx-auto py-10">
	<div class="py-5 flex justify-end">
		<Button on:click={createSeatServices} class="bg-primary-dark hover:bg-primary-50 flex gap-2">
			<svg
				width="20px"
				height="20px"
				viewBox="0 0 24 24"
	...
	<div class="overflow-x-auto">
		<table class="border-collapse w-full">
			...
			<tbody>
				{#each $seatServices as item, index (item.id)}
					<tr>
						...
						{#if item.seat_services_languages}
							...
							<td class="p-3 font- bg-gray-10 text-gray-600 border border-gray-200 table-cell">
								{#each item.seat_services_languages as lang}
									<div>
										{extractText(lang.description)?.slice(0, 40)}
									</div>
								{/each}
							</td>
						{/if}

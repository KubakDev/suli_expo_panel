...
const appendShapeToPlaceHolder = (
	placeHolder: string,
	d3: any,
	image: string | null = null,
	seatWidth: number = 20,
	seatHeight: number = 20,
	seatX: number = 5,
	seatY: number = 5,
	seat: SeatDesignModel | null = null
) => {
	let svg: SVGSVGElement;

	const randomId = `seat-${Math.floor(Math.random() * 100000)}`;
	const clonedSeat: d3.Selection<SVGGElement, any, any, any> = d3
		.select(placeHolder)
		.append('g')
		// add random class
		.attr('id', randomId)
		...
		.attr(
			'transform',
			`translate(${seatX},${seatY}) rotate(${seat && seat.rotation ? seat.rotation : 0}, ${
				seatWidth / 2
			},  ${seatHeight / 2})`
		)
	...
	function dragged(event: any, seatGroup: d3.Selection<SVGGElement, any, any, any>) {
		...
		seatGroup.attr(
			'transform',
			`translate(${finlaX},${finlaY}) rotate(${currentRotation},${bbox.width / 2},${
				bbox.height / 2
			})`
		);
	...
	function rotate(event: any, clonedSeat: d3.Selection<SVGGElement, any, any, any>) {
		...
		clonedSeat
			...
			.attr(
				'transform',
				`translate(${translateX},${translateY}) rotate(${newRotation}, ${boxWidth / 2}, ${
					boxHeigh / 2
				})`
			);
